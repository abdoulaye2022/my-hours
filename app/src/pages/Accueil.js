...
const Accueil = () => {
    ...
    const dayHours = (shifts) => {
        ...
        shifts.filter(p => p.statut_shift === 1).map((p, i) => {
            var now = moment();
    ...
    const weekHours = (shifts) => {
        ...
        shifts.filter(p => p.statut_shift === 1).map((p, i) => {
            ...
            let start_shift = moment(p.start_date, "YYYY-MM-DD HH:mm");
            let end_shift = moment(p.end_date, "YYYY-MM-DD HH:mm");
            let sun = moment(sunday).subtract(1, "days").format('YYYY-MM-DD');
        ...
        <Container fluid style={{ marginTop: 25 }}>
            <Row style={{ marginBottom: 25 }}>
                <Col  xs={12} md={12} lg={12} style={{ display: "flex", justifyContent: "center"}}>
                    <Button style={{ margin: "0px 10px" }} onClick={() => dispatch(shiftActions.shiftModal()) }>Ajouter un horaire</Button>
                    <Button style={{ margin: "0px 10px" }} onClick={() => navigate('/mes-horaires')}>Voir mes horaires</Button>
                </Col>
            </Row>
            <ShiftModal />
            <Row>
                <Col xs={12} md={4} lg={4}>
                    <Card style={{ width: "100%", textCenter: "center" }}>
                        <Card.Content style={{ textAlign: "center" }} header="Aujourd'hui" />
                        <Card.Content style={{ textAlign: "center" }} description={(
                            <Badge style={{ fontSize: "1.1em" }}>{dayHours(authShifts)}</Badge>
                        )} />
                    </Card>
                </Col>
                <Col xs={12} md={4} lg={4}>
                    <Card style={{ width: "100%", textCenter: "center" }}>
                        <Card.Content style={{ textAlign: "center" }} header="Cette semaine" />
                        <Card.Content style={{ textAlign: "center" }} description={(
                            <Badge style={{ fontSize: "1.1em" }}>{weekHours(authShifts)}</Badge>
                        )} />
                    </Card>
                </Col>
                <Col xs={12} md={4} lg={4}>
                    <Card style={{ width: "100%", textCenter: "center" }}>
                        <Card.Content style={{ textAlign: "center" }} header="Ce mois" />
                        <Card.Content style={{ textAlign: "center" }} description={(
                            <Badge style={{ fontSize: "1.1em" }}>{monthHours(authShifts)}</Badge>
                        )} />
                    </Card>
                </Col>
            </Row>
